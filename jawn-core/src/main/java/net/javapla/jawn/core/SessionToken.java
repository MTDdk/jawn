...
public interface SessionToken {
     ...
     */
    Cookie SESSION_COOKIE = new Cookie("jawn.session", null)
        .maxAge(-1)
        .httpOnly(true)
        .path("/")
        ;
    ...
    class CookieToken implements SessionToken {
        ...
        public String findToken(Context ctx) {
            String c = ctx.req().cookies().get(cookie.name());
        ...
        public void saveToken(Context ctx, String token) {
            ctx.resp().cookie(cookie.clone(token));
        }

        @Override
        public void deleteToken(Context ctx, String token) {
            // Is setting the value even necessary?
            // Perhaps it is here and not in a SecureSessionToken
            ctx.resp().cookie(cookie.clone(/*token*/).maxAge(0));
        }
